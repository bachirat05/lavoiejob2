<x-profile-layout :user="$user">
    <div class="card mb-6">
        <h5 class="card-header">Mettre à jour {{ $user->name }}</h5>
        <div class="card-body pt-1">
            <div id="wizard-property-listing" class="bs-stepper linear mt-2">
                ...
                <div class="step" data-target="#price-details">
                    <button type="button" class="step-trigger">
                    <span class="bs-stepper-circle"><i class="icon-base ti tabler-star icon-md"></i></span>
                    
                    </button>
                </div>
                </div>
                <div class="bs-stepper-content">
                <form id="wizard-property-listing-form"  action="{{ route('client.new_particulier') }}" class="hundle-form" method="post" enctype="multipart/form-data">
                    @csrf
                    <!-- Personal Details -->
              ...
              <label class="form-label" for="plFirstName">Nom du client</label>
              <input
                type="text"
                id="plFirstName"
                name="name"
            ...
            <div class="col-sm-6 form-control-validation">
              <p class="mb-3">Statut du compte</p>
              ...
              <div class="form-check my-2 ms-2">
                <input
                  ...
                  value="{{ $status->id }}"
                  {{ $user->statuses[0]->label == $status->label ? 'checked' : '' }}
                    />
                ...
                <div class="d-flex align-items-center lh-1 me-4 mb-4 mb-sm-0">
                            ...
                            <label class="form-check-label" for="status_{{ $status->name }}">{{ $status->label }}</label>
                        </div>
              </div>
              @endforeach
              @endif
            ...
            <div class="col-sm-6 form-control-validation">
              ...
              @if ($projets && count($projets)>0)
                        ...
                        <li class="d-flex align-items-start mb-4">
                            ...
                            <div class="d-flex justify-content-between w-100 flex-wrap gap-2">
                                ...
                                <div class="form-check form-check-inline">
                                    <input name="projets[]" class="form-check-input" type="checkbox" value="{{ $projet->id }}" id="inputpro{{ $projet->id }}" {{ in_array($projet->name, $user->projets->pluck('name')->toArray()) ? 'checked' : "" }} />
                                </div>
                                </div>
                            </div>
                        </li>
                  ...
                  @else
                      <p>Créer un nouveau projet pour affecter vos utilisateurs</p>
                  @endif
            </div>
                    ...
                    <div class="row g-6">
                        ...
                        <input
                            ...
                            id="cin_address"
                            name="cin_address"
                            value="{{ $user->userinfo->cin_address }}"
                            class="form-control"
                        ...
                        <input
                            ...
                            id="cin_validity"
                            name="cin_validity"
                            value="{{ $user->userinfo->cin_validity }}"
                        ...
                        <select
                            id="birth_city"
                            name="birth_city"
                            value="{{ $user->userinfo->birth_city }}"
                        ...
                        <input
                            ...
                            id="birth_date"
                            name="birth_date"
                            value="{{ $user->userinfo->birth_date }}"
                            class="form-control date-input" />
                        </div>
                        ...
                        <select
                            ...
                            class="select2 form-select"
                            data-allow-clear="true">
                            <option value="Appartement" selected="{{ $user->userinfo->logement == "Appartement" }}">Appartement</option>
                            <option value="Villa"  selected="{{ $user->userinfo->logement == "Villa" }}">Villa</option>
                            <option value="Maison"  selected="{{ $user->userinfo->logement == "Maison" }}">Maison</option>
                            <option value="Duplex"  selected="{{ $user->userinfo->logement == "Duplex" }}">Duplex</option>
                            <option value="Chalet"  selected="{{ $user->userinfo->logement == "Chalet" }}">Chalet</option>
                        </select>
                        </div>
                        <div class="col-sm-6 form-control-validation">
                        ...
                        <textarea
                            ...
                            class="form-control"
                            rows="2"
                            placeholder="APP 12, IMM 93, COMPLEXE NASIM (devant la pharmacie Nassim)">{{ $user->userinfo->address }}</textarea>
                        </div>
                    ...
                    <div class="row g-6">
                        ...
                        <select
                            ...
                            class="select2 form-select"
                            data-allow-clear="true">
                            <option value="Célibataire" selected="{{ $user->userinfo->marital == "Célibataire" }}">Célibataire</option>
                            <option value="Marié(e)" selected="{{ $user->userinfo->marital == "Marié(e)" }}">Marié(e)</option>
                            <option value="Divorcé(e)" selected="{{ $user->userinfo->marital == "Divorcé(e)" }}">Divorcé(e)</option>
                            <option value="Veuf(ve)" selected="{{ $user->userinfo->marital == "Veuf(ve)" }}">Veuf(ve)</option>
                        </select>
                        </div>
                        <div class="col-sm-3 form-control-validation">
                        <label class="form-label d-block" for="kids_number">Nombre d'enfants</label>
                        <input type="number" id="kids_number" name="kids_number" min="0" class="form-control" placeholder="3" value="{{ $user->userinfo->kids['kids_number'] }}" />
                        </div>
                        <div class="col-sm-3 form-control-validation">
